...
function crowdForRoute(route, index = 0) {
  ...
  const seed =
    `${route.route}-${route.route_id}-${index}`;
...
fs.createReadStream(
  ...
  .on('end', () => {
    ...
    console.log(
      `CSV loaded successfully: ${routes.length} routes`
    );
...
app.post('/chatbot', (req, res) => {
  ...
  if (!message) {
    ...
    return res.json({
      ...
      reply:
        'Ask me about routes, ETA, tracking or crowd prediction.',
